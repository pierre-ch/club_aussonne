<?php
	...
	{	$action = $_GET['action'];
		...
		switch ($vue)
		{	case "Equipe" :
				actionEquipe($action,$toutesLesEquipes);	break;
			case "Adherent" :
				actionAdherent($action,$tousLesAdherents);	break;	}
	}
	...
	function actionAdherent($action,$tousLesAdherents)
	{	switch ($action)
		{	case "ajouter":
				echo "je suis avant la création";
				$tousLesAdherents->ajouterUnAdherent(1,'Dupont','Pierre',8,'F');
				$tousLesAdherents->ajouterUnAdherent(2,'Dubois','Vincent',10,'M');
				$tousLesAdherents->ajouterUnAdherent(3,'Durant','Jacques',6,'M');
				$tousLesAdherents->ajouterUnAdherent(4,'Fleur','Sophie',7,'F');
				$tousLesAdherents->ajouterUnAdherent(5,'Martin','Valerie',8,'F');
				$tousLesAdherents->ajouterUnAdherent(6,'Dulac','Sandrine',12,'F');
				$_SESSION["tousLesAdherents"] = serialize($tousLesAdherents);
				echo "création effectué";	echo '<A href =menu.php>Retour</A>';	break;
			case "visualiser" :
